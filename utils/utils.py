...
class Measure:
    ...
    def ssim(self, imgA, imgB):
        # Handle small images by adjusting win_size
        min_dim = min(imgA.shape[0], imgA.shape[1])
        win_size = min(7, min_dim) if min_dim >= 3 else 3
        if win_size % 2 == 0:
            win_size -= 1
        
        try:
            score, diff = ssim(imgA, imgB, full=True, channel_axis=2, data_range=255, win_size=win_size)
        except:
            # Fallback for very small images
            score, diff = ssim(imgA, imgB, full=True, channel_axis=2, data_range=255, win_size=3)
        return score
